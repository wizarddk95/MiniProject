 ...
 * (2022.05.07)
 * 추가하고 싶은 기능들 
 * 1. Scanner 객체의 메소드로 입력값을 받을 때 다른 타입이 들어오면 다시 적으라고 출력하기. (현재는 에러가 떠서 프로그램이 멈춤) (추가 완료 - 22.05.08)
 * 2. "학생 정보 추가" 에서 특정 키, 예를 들면 q 를 입력했을 때까지는 계속 추가할 수 있게 하는 기능. (추가 완료 - 22.05.07)
 * 
 * 알게 된 점.
 ...
 * 5. 실수로 이름을 숫자로 받았는데 학생 정보 삭제를 하게 되니 java.util.ConcurrentModificationException 에러가 뜸.
 * 해당 Exception은 컬렉션 객체를 for loop를 사용해서 순회함과 동시에, 컬렉션 객체를 수정했기 때문에 발생한다고 한다.(꼭 숫자여서 뜬게 아니라 iterator와 관련있는듯)
 * 에러는 떴지만 로직은 잘 처리되었기 때문에 예외처리하였다.
 */
